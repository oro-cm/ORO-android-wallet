...
public class BlockchainServiceImpl extends android.app.Service implements BlockchainService
{
	...
	private void notifyCoinsReceived(@Nullable final Address address, final Coin amount)
	{
		...
		@SuppressLint({"StringFormatInvalid", "LocalSuppress"}) final String tickerMsg = getString(R.string.notification_coins_received_msg, btcFormat.format(amount)) + msgSuffix;
	...
	public void onCreate()
	{
		...
		} catch (final OroSystemException x) {
			throw new Error("blockchain cannot be created due to oro system error", x);
		}
