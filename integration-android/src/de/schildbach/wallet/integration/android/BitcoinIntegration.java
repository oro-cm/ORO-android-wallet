...
public final class BitcoinIntegration
{
	 ...
	 *            Calling Android activity
	 * @param requestCode
	 *            Code identifying the call when  is called back
	 * @param address
	 *            Bitcoin address
	 */
	...
	private static final int SATOSHIS_PER_COIN = 1000;
	...
	private static Intent makeBitcoinUriIntent(final String address, final Long amount)
	{
		...
		if (address != null)
			uri.append(address);
		if (amount != null)
			uri.append("?amount=").append(String.format("%d.%03d", amount / SATOSHIS_PER_COIN, amount % SATOSHIS_PER_COIN));
